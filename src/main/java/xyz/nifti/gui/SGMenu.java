package xyz.nifti.gui;

import org.bukkit.Material;
import org.bukkit.configuration.ConfigurationSection;
import xyz.nifti.gui.buttons.SGButton;
import xyz.nifti.gui.buttons.SGClickListener;
import xyz.nifti.gui.buttons.SGCloseListener;
import xyz.nifti.gui.buttons.SGDragItemListener;
import xyz.nifti.gui.item.ItemBuilder;
 ...
 * call to {@link MenuAPI#create(String, int)} (or {@link MenuAPI#create(String, int, String)})
 * from your plugin's {@link MenuAPI} instance.
 * <br><br>
 * This creates an inventory that is already associated with your plugin.
 * The reason for this is explained in the {@link MenuAPI#MenuAPI(JavaPlugin)}
...
public class SGMenu implements InventoryHolder {
    ...
    private SGCloseListener onClose;
    private CloseStatus closeStatus = CloseStatus.DEFAULT;
    private SGClickListener onClick;
    private SGDragItemListener onDragItem;
     ...
     * @param menuAPI The SpiGUI that created this inventory.
     * @param name The display name of the inventory.
     * @param rowsPerPage The number of rows per page.
     * @param tag The inventory's tag.
     */
    SGMenu(JavaPlugin owner, MenuAPI menuAPI, String name, int rowsPerPage, String tag) {
        this.owner = owner;
    ...
    /**
     * This is a per-inventory version of {@link MenuAPI#setBlockDefaultInteractions(boolean)}.
     *
     * @see MenuAPI#setBlockDefaultInteractions(boolean)
    ...
    /**
     * This is a per-inventory version of {@link MenuAPI#areDefaultInteractionsBlocked()}.
     *
     * @see MenuAPI#areDefaultInteractionsBlocked()
    ...
    /**
     * This is a per-inventory version of {@link MenuAPI#setEnableAutomaticPagination(boolean)}.
     * If this value is set, it overrides the per-plugin option set in {@link MenuAPI}.
     *
     * @see MenuAPI#setEnableAutomaticPagination(boolean)
    ...
    /**
     * This is a per-inventory version of {@link MenuAPI#isAutomaticPaginationEnabled()}.
     *
     * @see MenuAPI#isAutomaticPaginationEnabled()
    ...
    /**
     * This is a per-inventory version of ({@link MenuAPI#setDefaultPaginationButtonBuilder(SGPaginationButtonBuilder)}).
     *
     * @see MenuAPI#setDefaultPaginationButtonBuilder(SGPaginationButtonBuilder)
    ...
    /**
     * This is a per-inventory version of ({@link MenuAPI#getDefaultPaginationButtonBuilder()}).
     *
     * @see MenuAPI#getDefaultPaginationButtonBuilder()
     ...
     * <br><br>
     * The tag is used when getting all open inventories ({@link MenuAPI#findOpenWithTag(String)}) with your chosen tag.
     ...
     *
     * @see #getTag()
     * @see MenuAPI#findOpenWithTag(String)
    ...
    /**
     * Create new {@link SGButton} with {@link MenuAPI#getDefaultIcon default icon}.
     * <p>
     * Automatically applies name and lore from {@code config}
     *
     * @param config config to get lore and name for {@link SGButton}
     * @return new button with default icon
     *
     * @see MenuAPI#getDefaultIcon
     */
    public SGButton button(ConfigurationSection config) {
        ItemStack icon;
        String material = config.getString("material", "null");
    
        try {
            icon = new ItemBuilder(Material.valueOf(material))
                    .name(config.getString("name", "null"))
                    .lore(config.getStringList("lore"))
                    .build();
        } catch (IllegalArgumentException e) {
            icon = new ItemBuilder(menuAPI.getDefaultIcon().clone())
                    .name(config.getString("name", "null"))
                    .lore(config.getStringList("lore"))
                    .build();
        }
     ...
     * Create new {@link SGButton} with {@link MenuAPI#getDefaultIcon default icon}.
     * <p>
     * Automatically formats and applies name and lore from {@code config}.
     *
     * @param config config to get lore and name for {@link SGButton}
     * @param placeholders placeholders to replace in name and lore
     * @return new button with default icon
     *
     * @see MenuAPI#getDefaultIcon
     */
    public SGButton button(ConfigurationSection config, Map<String, String> placeholders) {
        ItemStack icon;
        String name = Objects.requireNonNull(config.getString("name", "null"));
        List<String> lore = config.getStringList("lore");
        String material = config.getString("material", "null");

        for (Map.Entry<String, String> placeholder : placeholders.entrySet()) {
            name = name.replaceAll(placeholder.getKey(), placeholder.getValue());
            lore = lore.stream().map(line -> line.replaceAll(placeholder.getKey(), placeholder.getValue())).collect(Collectors.toList());
        }
     ...
     */
    public synchronized void addButton(SGButton button) {
     ...
     */
    public synchronized void addButtons(SGButton... buttons) {
    ...
    /**
     * @see #setOnClose(SGCloseListener)
     * @return The action to be performed on close.
     */
    public SGCloseListener getOnClose() {
        return this.onClose;
    }
     ...
     */
    public void setOnClose(SGCloseListener onClose) {
        this.onClose = onClose;
    }

    /**
     * @see #setCloseStatus(CloseStatus)
     * @return The status for close
     */
    ...
    /**
     * Used to set a close status <br>
     * Determines why inventory was closed
     *
     * @param closeStatus Status for close.
     */
    ...
    /**
     * @see #setOnClick(SGClickListener)
     * @return The action to be performed on click.
     */
    public SGClickListener getOnClick() {
        return this.onClick;
    }

    /**
     * Used to set an action to be performed on inventory click without
     * registering an {@link org.bukkit.event.inventory.InventoryClickEvent} specifically
     * for this inventory.
     *
     * @param onClick The action to be performed on click.
     */
    public void setOnClick(SGClickListener onClick) {
        this.onClick = onClick;
    }

    /**
     * @see #setOnDragItem(SGDragItemListener)
     * @return The action to be performed on item move.
     */
    public SGDragItemListener getOnDragItem() {
        return this.onDragItem;
    }

    /**
     * Used to set an action to be performed on move item in inventory without
     * registering an {@link org.bukkit.event.inventory.InventoryMoveItemEvent} specifically
     * for this inventory.
     *
     * @param onDragItem The action to be performed on move item.
     */
    public void setOnDragItem(SGDragItemListener onDragItem) {
    ...
    public void fillBorders(ItemStack item) {
        fillBorders(new SGButton(item));
    }

    public void fillBorders(SGButton item) {
        for (int index = 0; index < getPageSize(); index++) {
            int row = index / 9;
            int column = (index % 9) + 1;

            if (row == 0 || row == getRowsPerPage() - 1 || column == 1 || column == 9) {
                setButton(index, item);
                stickSlot(index);
            }
        }
    }
    ...
    public Inventory getInventory() {
        boolean isAutomaticPaginationEnabled = menuAPI.isAutomaticPaginationEnabled();
        ...
        Inventory inventory = Bukkit.createInventory(this, (getPageSize()),
            name.replace("{currentPage}", String.valueOf(currentPage + 1))
                .replace("{maxPage}", String.valueOf(getMaxPage()))
        );

        // Add the main inventory items.
        for (int slot = currentPage * getPageSize(); slot < (currentPage + 1) * getPageSize(); slot++) {
            // If we've already reached the maximum assigned slot, stop assigning
            // slots.
            if (slot > getHighestFilledSlot()) break;

            if (items.containsKey(slot))
                getButton(slot).put(inventory, slot - (currentPage * getPageSize()));
        }

        // Update the stickied slots.
        for (int stickiedSlot : stickiedSlots)
            getButton(stickiedSlot).put(inventory, stickiedSlot);
        ...
        if (needsPagination) {
            ...
            for (int slot = pageSize - 9; slot < pageSize; slot++) {
                int offset = slot - pageSize;
                ...
                if (paginationButton != null)
                    paginationButton.put(inventory, slot);
            }
        }
